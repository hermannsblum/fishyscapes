...
@ex.command
def anomaly_segmentation_yt_2022(testing_dataset, _run, _log, validation=False):
    # added import inside the function to prevent conflicts if this method is not being tested

    from new_anomaly_segment.test import get_anomaly_detector
    from new_anomaly_segment.config.config import config
    import numpy as np
    import torch
    
    anomaly_segmentor = get_anomaly_detector(config.weight_path)
    anomaly_segmentor = anomaly_segmentor.cuda()
    ...
    _run.info['new_model_2022'] = fs.evaluate(wrapper, data)
