import sys
import os
sys.path.insert(0, os.path.join(os.getcwd(), os.path.dirname(os.path.dirname(__file__)), 'ood_func'))
from ood_func import ood_func
...
@ex.command
def ood_func(testing_dataset, _run, _log, ours=True, validation=False):
    # added import inside the function to prevent conflicts if this method is not being tested
    sys.path.insert(0, os.path.join(os.getcwd(), os.path.dirname(os.path.dirname(__file__)), 'ood_func'))
    from ood_func import ood_func
    model_name = "ood_func"
    #ood_func = network_manager.get_net()
    ...
    def wrapper(image):
        image = image.numpy().astype('uint8')
        anomaly_map = ood_func(image)
        return anomaly_score

    _run.info['ood_func'] = fs.evaluate(wrapper, data)
