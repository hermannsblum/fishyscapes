...
@ex.command
def k_segmentation(testing_dataset, _run, _log, validation=False):
    # added import inside the function to prevent conflicts if this method is not being tested
    sys.path.insert(0, os.path.join(os.getcwd(), os.path.dirname(os.path.dirname(__file__)), 'm_kseg'))
    from kseg import network_wrapper
    detector = network_wrapper()
    ...
    def wrapper(image):
        image = image.numpy().astype('uint8')
        anomaly_score = detector.process_single_image(image)

        return anomaly_score

    _run.info['m_kseg'] = fs.evaluate(wrapper, data)
