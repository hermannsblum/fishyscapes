...
class PerPixelBaselineHead(nn.Module):
    ...
    def losses(self, predictions, targets):
        ...
        if torch.isnan(predictions).sum() > 0:
            print("Nan in predictions!")
            raise Exception('Loss predictions nan')
...
class PerPixelBaselinePlusHead(PerPixelBaselineHead):
    ...
    def layers(self, features):
        ...
        if torch.isnan(mask_features).sum()>0:
            print("Error in mask features")
        if torch.isnan(transformer_encoder_features).sum() > 0:
                print("Error in encoder features")
